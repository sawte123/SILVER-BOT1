...
setInterval(() => {
    http.get(`http://silverbot45.glitch.me/`); /// حط اسم المشروع تبعك name تعديل مهم بدل
}, 280000);
...
client.on("message", async message => {
    ...
    if (message.content.startsWith(prefix + "sil")) {
        ...
        let args = message.content
            ...
            .slice(1)
            .join(" ");
        if (!message.author.id === "766039094819029002") return; ///تعديل مهم حط الايدي تبعك
...
client.on("ready", () => {
    client.user.setActivity(`type ${prefix}SH ON TOP `, { ///تعديل مهم حالة البوت
        type: "SH ON TOP"
    }); ///تعديل حالة البوت
});
